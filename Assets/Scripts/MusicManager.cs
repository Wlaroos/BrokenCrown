...
public class MusicManager : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        _track01.clip = _gameMusic;
        _track02.clip = _deathMusic;
    ...
    public void SwapTrack(Boolean isBattle)
    {
        StopAllCoroutines();
        
        StartCoroutine(FadeTrack(isBattle));
    }

    private IEnumerator FadeTrack(Boolean isBattle)
    {
        if (_isFirst)
        {
            _track01.UnPause();
        ...
        else if(!isBattle && _isPlayingMenu)
        {
            _track02.UnPause();
            while(timeElapsed < _timeToFade)
            {
